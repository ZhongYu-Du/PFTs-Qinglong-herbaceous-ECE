...
### Composite SEM (site|block)

## Soil physics and chemistry
soil_PC <- lmer(Sb ~ soilPC1 + soilPC2 + (site|block) + (1|species), data = data_scale_sem)
summary(soil_PC)
Anova(soil_PC)
coefs(soil_PC, standardize = "scale")

beta_soil_pc1 <- summary(soil_PC)$coefficients[2,1]
beta_soil_pc2 <- summary(soil_PC)$coefficients[2,2]

data_scale_sem$soilPC <- beta_soil_pc1 * data_scale_sem$soilPC1 + beta_soil_pc2 * data_scale_sem$soilPC2

## Soil HMs
soil_HM <- lmer(Sb ~ soilHMsPC1 + (site|block) + (1|species), data = data_scale_sem)
coefs(soil_HM, standardize = "scale")

## Plant mor
plant_mor <- lmer(Sb ~ pmorPC1 + pmorPC2 + pmorPC3 + (site|block)+ (1|species), data = data_scale_sem)
summary(plant_mor)
Anova(plant_mor)
coefs(plant_mor, standardize = "scale")

beta_plant_mor_pc1 <- summary(plant_mor)$coefficients[2,1]
beta_plant_mor_pc2 <- summary(plant_mor)$coefficients[2,2]
beta_plant_mor_pc3 <- summary(plant_mor)$coefficients[2,3]

data_scale_sem$plant_mor <- beta_plant_mor_pc1 * data_scale_sem$pmorPC1 + beta_plant_mor_pc2 * data_scale_sem$pmorPC2 + beta_plant_mor_pc3 * data_scale_sem$pmorPC3


## Plant ele
plant_ele <- lmer(Sb ~ pelePC1 + pelePC2 + (site|block)+ (1|species), data = data_scale_sem)
summary(plant_ele)
Anova(plant_ele)
coefs(plant_ele, standardize = "scale")

beta_plant_ele_pc1 <- summary(plant_ele)$coefficients[2,1]
beta_plant_ele_pc2 <- summary(plant_ele)$coefficients[2,2]

data_scale_sem$plant_ele <- beta_plant_ele_pc1 * data_scale_sem$pelePC1 + beta_plant_ele_pc2 * data_scale_sem$pelePC2


## Plant ratio
plant_rat <- lmer(Sb ~ pratPC1 + pratPC2 + (site|block) + (1|species), data = data_scale_sem)
summary(plant_rat)
Anova(plant_rat)
coefs(plant_rat, standardize = "scale")

beta_plant_rat_pc1 <- summary(plant_rat)$coefficients[2,1]
beta_plant_rat_pc2 <- summary(plant_rat)$coefficients[2,2]

data_scale_sem$plant_rat <- beta_plant_rat_pc1 * data_scale_sem$pratPC1 + beta_plant_rat_pc2 * data_scale_sem$pratPC2

head(data_scale_sem)

## SEM Sb/As
mod1 <- psem(
  lmer(Sb ~ soilPC + soilHMsPC1 + plant_mor + plant_ele + plant_rat + (site|block) + (1|species), data = data_scale_sem),
  lmer(As ~ soilPC + soilHMsPC1 + plant_mor + plant_ele +  (site|block) + (1|species), data = data_scale_sem),
  
  lmer(plant_mor ~ soilPC +  (site|block) + (1|species), data = data_scale_sem),
  lmer(plant_ele ~ soilPC + soilHMsPC1 + plant_mor + (site|block) + (1|species), data = data_scale_sem),
  lmer(plant_rat ~ soilPC + soilHMsPC1 + plant_ele + (site|block) + (1|species), data = data_scale_sem),
...
summary(mod1, .progressBar = F)
AIC(mod1, AIC.type = "dsep")

require(semEff) ###
require(flextable)


### Composite SEM (1|block)

## Soil physics and chemistry
soil_PC <- lmer(Sb ~ soilPC1 + soilPC2 + (1|block) + (1|species), data = data_scale_sem)
summary(soil_PC)
Anova(soil_PC)
coefs(soil_PC, standardize = "scale")
...
## Soil HMs
soil_HM <- lmer(Sb ~ soilHMsPC1 + (1|block) + (1|species), data = data_scale_sem)
coefs(soil_HM, standardize = "scale")

## Plant mor
plant_mor <- lmer(Sb ~ pmorPC1 + pmorPC2 + pmorPC3 + (1|block)+ (1|species), data = data_scale_sem)
summary(plant_mor)
Anova(plant_mor)
coefs(plant_mor, standardize = "scale")
...
## Plant ele
plant_ele <- lmer(Sb ~ pelePC1 + pelePC2 + (1|block)+ (1|species), data = data_scale_sem)
summary(plant_ele)
Anova(plant_ele)
coefs(plant_ele, standardize = "scale")
...
## Plant ratio
plant_rat <- lmer(Sb ~ pratPC1 + pratPC2 + (1|block) + (1|species), data = data_scale_sem)
summary(plant_rat)
Anova(plant_rat)
coefs(plant_rat, standardize = "scale")
...
## SEM Sb/As
mod2 <- psem(
  lmer(Sb ~ soilPC + soilHMsPC1 + plant_mor + plant_ele + plant_rat + (1|block) + (1|species), data = data_scale_sem),
  lmer(As ~ soilPC + soilHMsPC1 + plant_mor + plant_ele +  (1|block) + (1|species), data = data_scale_sem),
  
  lmer(plant_mor ~ soilPC +  (1|block) + (1|species), data = data_scale_sem),
  lmer(plant_ele ~ soilPC + soilHMsPC1 + plant_mor + (1|block) + (1|species), data = data_scale_sem),
  lmer(plant_rat ~ soilPC + soilHMsPC1 + plant_ele + (1|block) + (1|species), data = data_scale_sem),
...
summary(mod2, .progressBar = F)
AIC(mod2, AIC.type = "dsep")
